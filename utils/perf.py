...
def print_throughput(hf_model, args, e2e_time, rank=0):
    if rank <= 0:
        ...
        batch_size = args.per_device_train_batch_size * gpus_per_model
        ...
        partition_param_num = sum([
            p.partition_numel() if hasattr(p, "ds_tensor") else p.numel()
            for p in hf_model.parameters()
        ])
        ...
        print(
            f"Model Parameters: {param_string}, Partition Model Parameters: {partition_param_num}, Latency: {e2e_time:.2f}s, TFLOPs: {train_tflops:.2f}, Samples/sec: {samples_per_second:.2f}, Time/seq {e2e_time/batch_size:.2f}s, Batch Size: {batch_size}, Sequence Length: {seq_length}"
        )
...
def calculate_flops(checkpoint_activations_factor, batch_size, seq_length,
                    hf_config):
    num_layers, hidden_size, vocab_size = get_hf_configs(hf_config)
    print('hyper_parameter', num_layers, hidden_size, vocab_size, batch_size, seq_length, checkpoint_activations_factor)
